...
private struct BBCodeTextKitRenderer {
  ...
  func renderBlocks(root: BBCodeNode) -> [BBCodePreparedBlock] {
  ...
  private func renderBlocks(children: [BBCodeNode]) -> [BBCodePreparedBlock] {
  ...
  private func renderSegments(children: [BBCodeNode]) -> [RenderedSegment] {
    normalizeSegments(children.flatMap(renderSegments(node:)))
  }

  private func renderSegments(node: BBCodeNode) -> [RenderedSegment] {
  ...
  private func applyNodeWrapper(_ node: BBCodeNode, to segments: [RenderedSegment]) -> [RenderedSegment] {
    switch node.type {
    case .center:
  ...
  private func renderNode(_ node: BBCodeNode) -> NSMutableAttributedString {
    switch node.type {
    case .plain:
  ...
  private func mediaPayload(for node: BBCodeNode) -> BBCodePreparedBlock.Payload? {
    switch node.type {
    case .image:
      let link = node.renderInnerHTML(nil).trimmingCharacters(in: .whitespacesAndNewlines)
      guard let safeLink = bbcodeSafeURLString(url: link, defaultScheme: "https", defaultHost: nil),
        let url = URL(string: safeLink)
      else {
        return nil
      }
  ...
  private func renderAlignedBlock(node: BBCodeNode, alignment: NSTextAlignment)
  ...
  private func renderList(_ node: BBCodeNode) -> NSMutableAttributedString {
  ...
  private func renderCode(_ node: BBCodeNode) -> NSMutableAttributedString {
    let inner = renderChildren(node.children)
    trimLeadingNewlines(in: inner)
    trimTrailingNewlines(in: inner)
    applyFontTransform(to: inner) { font in
  ...
  private func renderQuote(_ node: BBCodeNode) -> NSMutableAttributedString {
    let inner = renderChildren(node.children)
    trimLeadingNewlines(in: inner)
    trimTrailingNewlines(in: inner)
  ...
  private func renderUser(_ node: BBCodeNode) -> NSMutableAttributedString {
    let inner = renderChildren(node.children)
    trimLeadingNewlines(in: inner)
    trimTrailingNewlines(in: inner)
  ...
  private func renderURL(_ node: BBCodeNode) -> NSMutableAttributedString {
    let inner = renderChildren(node.children)
    trimLeadingNewlines(in: inner)
    trimTrailingNewlines(in: inner)
  ...
  private func renderRuby(_ node: BBCodeNode) -> NSMutableAttributedString {
  ...
  private func renderSmiley(_ node: BBCodeNode) -> NSMutableAttributedString {
    guard let smiley = BBCodeSmileyCatalog.item(for: node.attr),
    ...
    let attachment = BBCodeSmileyTextAttachment(
      item: smiley,
      resourcePath: path,
  ...
  private func renderBmo(_ node: BBCodeNode) -> NSMutableAttributedString {
    let result = BBCodeBmoDecoder.decode(node.attr)
    guard !result.items.isEmpty,
      let cgImage = BBCodeBmoRenderer.renderCGImage(from: result, textSize: Int(textSize) + 4)
    else {
      return makeText("(\(node.attr))")
    }

    let attachment = BBCodeInlineImageTextAttachment(
      image: UIImage(cgImage: cgImage), size: CGSize(width: textSize + 4, height: textSize + 4))
    return makeInlineAttachmentText(attachment)
  }

  private func collectListItemNodes(from children: [BBCodeNode]) -> [[BBCodeNode]] {
    var items: [[BBCodeNode]] = []
    var current: [BBCodeNode] = []
  ...
  private func inlineSmileySize(smiley: BBCodeSmileyItem, sourceSize: CGSize) -> CGSize {
  ...
  private func parseColor(_ rawValue: String) -> UIColor? {
    ...
    if let alias = BBCodeWebColorAlias.allCases.first(where: { String(describing: $0) == color }) {
      color = String(describing: alias.standardColor)
    }

    if let webColor = BBCodeWebColor.allCases.first(where: { String(describing: $0) == color }) {
...
final class BBCodeSmileyTextAttachment: NSTextAttachment {
  ...
  init(item: BBCodeSmileyItem, resourcePath: String, placeholderImage: UIImage, displaySize: CGSize) {
    self.item = item
...
final class BBCodeInlineImageTextAttachment: NSTextAttachment {
  let renderedSize: CGSize
