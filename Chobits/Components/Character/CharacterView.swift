...
struct CharacterView: View {
  ...
  var birthday: String {
    var text = ""
    if let year = character?.birthYear {
      text += "\(year)年"
    }
    if let month = character?.birthMonth {
      text += "\(month)月"
    }
    if let day = character?.birthDay {
      text += "\(day)日"
    }
    return text
  }
            ...
            HStack(alignment: .top) {
              ...
              VStack(alignment: .leading) {
                ...
                if let bloodType = character.bloodType {
                  Label("血型: \(BloodType(bloodType).name)", systemImage: "heart.fill").foregroundStyle(.secondary)
                }
                if !birthday.isEmpty {
                  Label("生日: \(birthday)", systemImage: "calendar").foregroundStyle(.secondary)
                }
                Spacer()
                HStack {
